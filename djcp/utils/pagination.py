...
class Pagination(object):
    ...
    def html(self):
        ...
        for i in range(before_page, after_page + 1):
            ...
            if self.page == i:
                li_str = '<li class="active"><a href="?{}" >{}</a></li>'.format(
                    self.query_dict.urlencode(), i, i)
            else:
                li_str = '<li><a href="?{}" >{}</a></li>'.format(
                    self.query_dict.urlencode(), i, i)
        ...
        if self.page < self.totle_page_count:
            self.query_dict.setlist(self.page_parm, [self.page + 1])
            prev_page = '<li class="next"><a href="?{}" >下一页</a></li>'.format(
                self.query_dict.urlencode())
        ...
        # 尾页
        self.query_dict.setlist(self.page_parm, [self.totle_page_count])
        prev_page = '<li ><a href="?{}">尾页</a></li>'.format(
            self.query_dict.urlencode())
        li_strlist.append(prev_page)
